...
async function fetchBlogMetadata(env: Env): Promise<Record<string, BlogMetadataEntry>> {
  if (cachedBlogMetadata) {
    return cachedBlogMetadata;
  }
  try {
    const response = await env.ASSETS.fetch(new Request('http://localhost/blog-metadata.json'));
    if (!response.ok) {
      throw new Error(`Failed to fetch blog-metadata.json: ${response.statusText}`);
    }
...
export async function getAllBlogPosts(env: Env): Promise<BlogPost[]> {
  try {
    ...
    return Object.entries(blogMetadata).map(([slug, data]: [string, BlogMetadataEntry]) => ({
      slug,
      title: data.title,
...
export async function getBlogPostBySlug(slug: string, env: Env): Promise<BlogPost | undefined> {
  try {
    const blogMetadata = await fetchBlogMetadata(env);
