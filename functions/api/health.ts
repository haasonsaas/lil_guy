...
async function performHealthCheck(
  ...
  const endpoints = [
    {
      ...
      status:
        endpointChecks[0].status === 'fulfilled'
          ? endpointChecks[0].value
          : 'down',
    },
    {
      ...
      status:
        endpointChecks[1].status === 'fulfilled'
          ? endpointChecks[1].value
          : 'down',
    },
    {
      ...
      status:
        endpointChecks[2].status === 'fulfilled'
          ? endpointChecks[2].value
          : 'down',
    },
    {
      name: 'Analytics API',
      ...
      status:
        endpointChecks[3].status === 'fulfilled'
          ? endpointChecks[3].value
          : 'down',
    },
  ] as const

  // Calculate overall service health
  const operationalCount = endpoints.filter(
  ...
  if (overallStatus === 'healthy') {
    ...
    message =
      'Some services experiencing issues. Core functionality may be limited.'
  } else {
  ...
  return {
    ...
    services: {
      ...
      analytics:
        endpoints.find((e) => e.name === 'Analytics API')?.status || 'down',
