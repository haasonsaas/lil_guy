...
const blogPosts = [
  ...
  },
  {
    ...
    description: 'If you\'re feeling like the startup funding landscape has shifted under your feet, you\'re not imagining it.',
  ...
  },
  {
    ...
    description: 'Examining why technical founders often confuse early signals with genuine product-market fit.',
  ...
  },
  {
    ...
    description: 'Most SaaS founders track LTV/CAC wrong. Here\'s what really drives sustainable growth.',
  ...
  },
  {
    ...
    description: 'After year four, founder CEOs are paid like caretakers—while hired CEOs are paid like kings.',
...
function calculateRecommendationScore(
  ...
  // Topic matching
  if (topic && post.topics.some(t => t.includes(topic) || topic.includes(t))) {
    score += 4;
...
export async function onRequest(context: EventContext<Env, string, Record<string, unknown>>): Promise<Response> {
  ...
  const scoredPosts = blogPosts.map(post => {
    const { score, reason, priority } = calculateRecommendationScore(post, role || undefined, topic || undefined, experience || undefined);
    return {
      ...post,
      score,
      reason,
  ...
  if (criteria.length === 0) {
    reasoning = 'General recommendations for startup professionals covering essential topics across product development, technical leadership, and business strategy.';
  } else {
    reasoning += criteria.join(', ') + '. Posts are ranked by relevance to your specific context and needs.';
  }
