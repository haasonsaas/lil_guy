...
export const onRequest: PagesFunction = async ({ request, next }) => {
  ...
  if (/-[a-f0-9]{8,}\./.test(pathname) && /\.(js|css|woff2?|png|jpg|jpeg|webp|avif|svg|ico)$/.test(pathname)) {
    response.headers.set('Cache-Control', 'public, max-age=31536000, immutable')
    response.headers.set('CF-Cache-Tag', 'static-assets')
  }
  // Generated blog images (long cache with revalidation)
  else if (pathname.startsWith('/generated/') || pathname.startsWith('/placeholders/')) {
  ...
  else if (/\.(js|css|woff2?|png|jpg|jpeg|webp|avif|svg|ico|pdf|txt|xml)$/.test(pathname)) {
  ...
  // Blog post pages (moderate cache with stale-while-revalidate)
    ...
    response.headers.set('Cache-Control', 'public, max-age=1800, s-maxage=7200, stale-while-revalidate=3600')
    response.headers.set('CF-Cache-Tag', 'blog-content')
  }
  // Main pages (home, about, blog index)
  else if (pathname === '/' || pathname === '/about' || pathname === '/blog' || pathname === '/experiments') {
  ...
  else if (pathname === '/rss.xml' || pathname === '/atom.xml' || pathname === '/sitemap.xml' || pathname === '/robots.txt') {
    response.headers.set('Cache-Control', 'public, max-age=3600, s-maxage=86400')
