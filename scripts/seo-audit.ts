...
async function performAudit(baseUrl: string): Promise<SEOAuditResult> {
  ...
  if (errorCount > 0) {
    recommendations.push(
      'Fix critical errors immediately (missing titles, descriptions, H1 tags)'
    )
  }
  if (warningCount > 3) {
    recommendations.push(
      'Address warning-level issues to improve SEO performance'
    )
  }
  if (meta.externalLinks > 0) {
    recommendations.push(
      'Consider adding rel="nofollow" to external links where appropriate'
    )
  }
  if (!meta.keywords && meta.description) {
    recommendations.push(
      'Research and target specific keywords for better ranking'
    )
  }
  if (values.depth === 'basic') {
    recommendations.push(
      'Run comprehensive audit with --depth=comprehensive for full analysis'
    )
  }
...
function generateTableReport(audit: SEOAuditResult): void {
  ...
  console.log(
    `${scoreColor(audit.score.toString())}/100 ${getScoreEmoji(audit.score)}`
  )
  console.log(`${chalk.cyan('URL:')} ${chalk.gray(audit.url)}`)
  console.log(
    `${chalk.cyan('Pages Analyzed:')} ${chalk.white(audit.stats.totalPages)}`
  )
  console.log(
    `${chalk.cyan('Timestamp:')} ${chalk.gray(new Date(audit.timestamp).toLocaleString())}`
  )
  ...
  if (audit.issues.length > 0) {
    ...
    const issuesByCategory = audit.issues.reduce(
      (acc, issue) => {
  ...
  console.log(
    chalk.gray('🚀 Run with --depth=comprehensive for full site audit')
  )
  console.log()
}
