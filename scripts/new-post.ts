...
if (titleArgs.length === 0) {
  ...
  console.error(
    'Usage: bun run new-post "My Post Title" [-d|--draft] [-t|--tags "tag1,tag2"] [--description "desc"]'
  )
  process.exit(1)
}
...
const generateFrontmatter = (
  title: string,
  options: { tags?: string; description?: string; draft?: boolean }
): string => {
...
async function createNewPost() {
  ...
  if (existsSync(filepath)) {
    console.error(
      `❌ Error: Post with slug "${slug}" already exists at ${filepath}`
    )
    process.exit(1)
  }
...
${body}`
  ...
  try {
    ...
    console.log(
      `${values.draft ? '📄 Status: Draft' : '🚀 Status: Ready to publish'}`
    )
    ...
    if (values.open) {
      ...
      try {
        ...
        try {
          ...
          console.log(
            '⚠️  Could not open file automatically. Please open manually.'
          )
        }
      }
    }
    ...
    if (values.dev) {
      ...
      console.log(
        `📖 Post will be available at: http://localhost:8080/blog/${slug}`
      )
