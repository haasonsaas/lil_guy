...
function updateCodeFiles(): number {
  ...
  for (const file of codeFiles) {
    ...
    content = content.replace(/\/generated\/([^"'`\s]+)\.png/g, '/generated/$1.webp');
 ...
 */
function removePNGFiles(dryRun: boolean = false): { removed: number; spaceSavedMB: number } {
  ...
  for (const pngFile of pngFiles) {
    if (hasWebPEquivalent(pngFile)) {
      ...
      if (!dryRun) {
        ...
        console.log(`🗑️  Removed: ${path.basename(pngFile)} (${Math.round(stats.size / 1024)}KB)`);
      } else {
        console.log(`🔍 Would remove: ${path.basename(pngFile)} (${Math.round(stats.size / 1024)}KB)`);
      }
      removed++;
    } else {
 ...
 */
async function migrate(options: { dryRun?: boolean; updateCode?: boolean } = {}) {
  ...
  console.log(`   PNG files ${dryRun ? 'would be' : ''} removed: ${stats.pngFilesRemoved}`);
  console.log(`   Code files updated: ${stats.codeFilesUpdated}`);
  console.log(`   Space ${dryRun ? 'would be' : ''} saved: ${stats.spaceSavedMB.toFixed(2)}MB`);
