...
async function generateSitemap() {
  try {
    ...
    console.log(
      chalk.green(`✅ Sitemap also generated in public: ${publicPath}`)
    )
    console.log(chalk.green(`✅ Robots.txt generated: ${distRobotsPath}`))
    console.log(
      chalk.green(`✅ Robots.txt also generated in public: ${publicRobotsPath}`)
    )
    ...
    if (sitemapSize > maxSize) {
      console.log(
        chalk.yellow(
          `⚠️  Warning: Sitemap size (${(sitemapSize / 1024 / 1024).toFixed(2)}MB) is large. Consider splitting into multiple sitemaps.`
        )
      )
    }
    ...
    if (allUrls.length > 50000) {
      console.log(
        chalk.yellow(
          `⚠️  Warning: Sitemap contains ${allUrls.length} URLs. Google recommends max 50,000 per sitemap.`
        )
      )
    }
  } catch (error) {
