...
const requiredSecrets: Secret[] = [
  {
    ...
    description: 'Cloudflare API token with Pages:Edit and Zone:Read permissions',
    example: 'abc123...',
    required: true
  },
  {
...
async function checkSecrets(): Promise<void> {
  ...
  try {
    ...
    const existingSecrets = stdout.split('\n')
    ...
    const missingRequired = requiredSecrets.filter(secret => 
      secret.required && !existingSecrets.includes(secret.name)
    );
...
async function interactiveSetup(): Promise<void> {
  ...
  for (const secret of requiredSecrets) {
    ...
    try {
      ...
      const { stdout } = await execAsync(`gh secret list | grep "^${secret.name}\\s" || echo ""`);
      if (stdout.trim()) {
