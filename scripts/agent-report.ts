...
function parsePeriod(period: string): number {
  ...
  if (!match) {
    console.error(chalk.red('❌ Invalid period format. Use format like "7d" or "2w"'));
    process.exit(1);
  }
...
async function fetchAnalytics(baseUrl: string, days: number): Promise<AnalyticsData> {
  try {
...
function formatEndpoint(endpoint: string): string {
  ...
  const color = endpointColors[endpoint as keyof typeof endpointColors] || chalk.white;
...
function generateTableReport(data: AnalyticsData): void {
  if (!data.stats) {
    ...
    console.log(chalk.gray('💡 Tip: AI agents need to interact with your APIs first.'));
    return;
  }
  ...
  if (stats.topEndpoints.length > 0) {
    ...
    stats.topEndpoints.forEach((endpoint, index) => {
      ...
      const bar = '█'.repeat(Math.max(1, Math.floor(endpoint.count / stats.topEndpoints[0].count * 20)));
  ...
  if (stats.topAgents.length > 0) {
    ...
    stats.topAgents.forEach((agent, index) => {
      ...
      const bar = '█'.repeat(Math.max(1, Math.floor(agent.count / stats.topAgents[0].count * 20)));
  ...
  if (stats.totalEvents === 0) {
    ...
    console.log(chalk.yellow('• Early adoption phase - promote to AI community'));
...
function generateMarkdownReport(data: AnalyticsData): void {
  ...
  if (stats.topEndpoints.length > 0) {
    ...
    stats.topEndpoints.forEach((endpoint, index) => {
      const percentage = ((endpoint.count / stats.totalEvents) * 100).toFixed(1);
      console.log(`${index + 1}. **${endpoint.endpoint}** - ${endpoint.count} requests (${percentage}%)`);
    });
    console.log();
  }
  ...
  if (stats.topAgents.length > 0) {
    ...
    stats.topAgents.forEach((agent, index) => {
      const percentage = ((agent.count / stats.totalEvents) * 100).toFixed(1);
      console.log(`${index + 1}. **${agent.agent}** - ${agent.count} requests (${percentage}%)`);
    });
    console.log();
  }
...
async function generateReport() {
  ...
  const data = await fetchAnalytics(values.baseUrl || 'https://haasonsaas.com', days);
