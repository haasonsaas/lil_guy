...
async function audit() {
  ...
  try {
    const { stdout: porcelainOutput } = await execAsync(
    ...
    const { stdout: allMarkdownFilesOutput } = await execAsync(
      "find src/posts -name '*.md'"
    )
    const allMarkdownFiles = allMarkdownFilesOutput.trim().split('\n')

    const markdownFilesToAudit = allMarkdownFiles.filter((file) =>
    ...
    console.log(
      chalk.yellow(
        `Found ${markdownFilesToAudit.length} staged markdown posts to audit:`
      )
    )
    markdownFilesToAudit.forEach((file) =>
