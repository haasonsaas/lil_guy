...
function calculateStreak(posts: Post[]): { current: number; longest: number } {
  ...
  const daysSinceLastPost = Math.floor((today.getTime() - lastPost.getTime()) / (1000 * 60 * 60 * 24));
  ...
  if (daysSinceLastPost > 7) {
    ...
    for (let i = 1; i < posts.length; i++) {
      const daysBetween = Math.floor((posts[i - 1].date.getTime() - posts[i].date.getTime()) / (1000 * 60 * 60 * 24));
      if (daysBetween <= 7) {
        currentHistorical++;
...
function getMomentum(daysSinceLastPost: number, postsLast30Days: number): MomentumData['momentum'] {
...
function getNextMilestone(currentStreak: number, totalPosts: number, totalWords: number): MomentumData['nextMilestone'] {
  const milestones = [
  ...
  const nextStreak = milestones[0].values.find(v => v > currentStreak) || currentStreak + 10;
  const nextPosts = milestones[1].values.find(v => v > totalPosts) || totalPosts + 25;
  const nextWords = milestones[2].values.find(v => v > totalWords) || totalWords + 10000;
...
function getNudge(data: MomentumData): string {
  ...
  if (data.currentStreak > 0 && data.currentStreak === data.longestStreak) {
    additions.push(`You're at your ALL-TIME BEST streak of ${data.currentStreak} posts!`);
  ...
  const progressPercent = Math.round((data.nextMilestone.current / data.nextMilestone.value) * 100);
  if (progressPercent > 80) {
    additions.push(`You're ${progressPercent}% to ${data.nextMilestone.value} ${data.nextMilestone.type}!`);
  }
...
async function analyzeMomentum(): Promise<MomentumData> {
  ...
  const { current: currentStreak, longest: longestStreak } = calculateStreak(posts);
  
  const lastPost = posts[0];
  const daysSinceLastPost = lastPost 
    ? Math.floor((now.getTime() - lastPost.date.getTime()) / (1000 * 60 * 60 * 24))
    : 999;
  ...
  const nextMilestone = getNextMilestone(currentStreak, posts.length, totalAllTimeWords);
...
function formatTable(data: MomentumData): void {
  ...
  console.log(`  Momentum: ${momentumEmoji[data.momentum]} ${momentumColor(data.momentum.toUpperCase())}`);
  console.log(`  Current Streak: ${chalk.green(data.currentStreak + ' posts')} ${data.currentStreak === data.longestStreak ? chalk.yellow('(Personal Best!)') : ''}`);
  console.log(`  Longest Streak: ${chalk.blue(data.longestStreak + ' posts')}`);
  console.log(`  Days Since Last Post: ${data.daysSinceLastPost === 0 ? chalk.green('Today!') : chalk.yellow(data.daysSinceLastPost + ' days')}`);
  console.log();
  ...
  console.log(`  Average: ${chalk.blue(Math.round(data.totalWords30Days / Math.max(data.totalPosts30Days, 1)) + ' words/post')}`);
  console.log();
  ...
  // Next milestone
  const progress = Math.round((data.nextMilestone.current / data.nextMilestone.value) * 100);
  const progressBar = '█'.repeat(Math.floor(progress / 5)) + '░'.repeat(20 - Math.floor(progress / 5));
  console.log(chalk.bold('Next Milestone'));
  console.log(`  ${data.nextMilestone.type}: ${data.nextMilestone.current} → ${data.nextMilestone.value}`);
...
function formatMinimal(data: MomentumData): void {
  const statusLine = data.momentum === 'on-fire' || data.momentum === 'hot' 
  ...
  console.log(`${statusLine} | ${data.totalPosts30Days} posts last 30d | ${progress}% to ${data.nextMilestone.value} ${data.nextMilestone.type}`);
