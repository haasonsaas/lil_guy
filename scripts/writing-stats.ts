...
async function calculateStats(
  ...
  const daysSincePeriodStart = since
    ...
    : posts.length > 0
      ? (new Date().getTime() -
          new Date(posts[posts.length - 1].frontmatter.pubDate).getTime()) /
        (1000 * 60 * 60 * 24)
      : 0
  ...
  return {
    ...
    averageWordsPerPost:
      filteredPosts.length > 0
        ? Math.round(totalWords / filteredPosts.length)
        : 0,
    shortestPost: sortedByWords[0] || { title: 'N/A', words: 0 },
    longestPost: sortedByWords[sortedByWords.length - 1] || {
      title: 'N/A',
      words: 0,
    },
...
function formatTable(stats: WritingStats, period: string): void {
  ...
  console.log(
    `  Total Posts: ${chalk.green(stats.totalPosts)} (${chalk.yellow(stats.publishedPosts)} published, ${chalk.gray(stats.totalDrafts)} drafts)`
  )
  console.log(
    `  Total Words: ${chalk.green(stats.totalWords.toLocaleString())}`
  )
  console.log(
    `  Average Words/Post: ${chalk.green(stats.averageWordsPerPost.toLocaleString())}`
  )
  console.log(
    `  Total Reading Time: ${chalk.green(stats.totalReadingTime)} minutes`
  )
  console.log()
  ...
  console.log(
    `  Shortest: "${chalk.cyan(stats.shortestPost.title)}" (${stats.shortestPost.words} words)`
  )
  console.log(
    `  Longest: "${chalk.cyan(stats.longestPost.title)}" (${stats.longestPost.words} words)`
  )
  console.log()
  ...
  if (stats.postsByMonth.length > 0) {
    ...
    stats.postsByMonth.slice(0, 6).forEach(({ month, count, words }) => {
      ...
      console.log(
        `  ${month}: ${chalk.green(count)} posts, ${chalk.blue(words.toLocaleString())} words (avg ${avgWords})`
      )
    })
  }
}
...
function formatMarkdown(stats: WritingStats, period: string): void {
  ...
  console.log(
    `- **Total Posts**: ${stats.totalPosts} (${stats.publishedPosts} published, ${stats.totalDrafts} drafts)`
  )
  console.log(`- **Total Words**: ${stats.totalWords.toLocaleString()}`)
  console.log(
    `- **Average Words/Post**: ${stats.averageWordsPerPost.toLocaleString()}`
  )
  ...
  console.log(
    `- **Shortest**: "${stats.shortestPost.title}" (${stats.shortestPost.words} words)`
  )
  console.log(
    `- **Longest**: "${stats.longestPost.title}" (${stats.longestPost.words} words)\n`
  )
  ...
  stats.postsByMonth.slice(0, 12).forEach(({ month, count, words }) => {
    const avgWords = Math.round(words / count)
    console.log(
      `| ${month} | ${count} | ${words.toLocaleString()} | ${avgWords} |`
    )
  })
}
