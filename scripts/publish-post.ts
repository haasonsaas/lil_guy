...
if (!query) {
  console.error(
    chalk.red('❌ Error: Please provide a search query to find the draft post')
  )
  console.error(
    'Usage: bun run publish-post "post title or keyword" [-d date] [-c] [-p]'
  )
  console.error('\nOptions:')
  console.error(
  ...
  console.error(
    '  -p, --push     Push to git after publishing (implies --commit)'
  )
  process.exit(1)
}
...
function updateFrontmatter(content: string, pubDate: string): string {
  ...
  if (!foundPubDate) {
    ...
    for (let i = 0; i < updatedLines.length; i++) {
      if (
        updatedLines[i].match(/^\s*author:\s*/) ||
        updatedLines[i].match(/^\s*title:\s*/)
      ) {
        insertIndex = i + 1
        break
      }
...
async function publishPost() {
  ...
  if (drafts.length > 1) {
    console.log(
      chalk.yellow(`Found ${drafts.length} draft posts matching "${query}":\n`)
    )
    ...
    drafts.slice(0, 5).forEach((draft, index) => {
      ...
      if (draft.description) {
        console.log(
          chalk.gray(`   📝 ${draft.description.substring(0, 80)}...`)
        )
      }
      console.log()
    })
  ...
  if (!values.commit && !values.push) {
    ...
    console.log(
      chalk.gray('   - Run "git add -A && git commit -m "publish: post title"')
    )
    ...
    console.log(
      chalk.gray('   - Your post will be live after the deployment completes!')
    )
  }
