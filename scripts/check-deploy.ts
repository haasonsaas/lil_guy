...
async function checkBuildSize(): Promise<CheckResult> {
  ...
  try {
    ...
    return {
      ...
      details: passed
        ? undefined
        : 'Consider optimizing your assets or using a CDN',
    }
  } catch (error) {
    return {
      passed: false,
...
async function checkCloudflareConfig(): Promise<CheckResult> {
  try {
    ...
    if (
      ...
      !content.includes('compatibility_date =')
    ) {
      return {
        passed: false,
...
async function checkDeployment() {
  ...
  if (allPassed) {
    console.log(
      chalk.green.bold('✨ All checks passed! Your deployment should succeed.')
    )
    console.log(chalk.gray('\nDeploy with: git push'))
  } else {
    console.log(
      chalk.red.bold('⚠️  Some checks failed. Review the issues above.')
    )
    console.log(
  ...
  console.log(
    chalk.gray('   - Check deployment logs at: https://dash.cloudflare.com')
  )
