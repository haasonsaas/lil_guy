...
interface ContentIdea {
  title: string;
  type: 'sequel' | 'deep-dive' | 'counter-point' | 'practical-guide' | 'case-study' | 'comparison';
  basedOn?: string;
  rationale: string;
...
function findContentGaps(posts: Post[], topTags: Map<string, number>): string[] {
  const gaps: string[] = [];
  ...
  for (let i = 0; i < tagArray.length; i++) {
    for (let j = i + 1; j < tagArray.length; j++) {
      ...
      const hasCombo = posts.some(p => 
        p.frontmatter.tags?.includes(tag1) && 
        p.frontmatter.tags?.includes(tag2)
      );
  ...
  const commonTopics = [
    ...
    'team-building', 'fundraising', 'customer-success', 'pricing-strategy',
    'competitive-analysis', 'market-research', 'user-research', 'design-systems'
  ];
  
  commonTopics.forEach(topic => {
    const hasTopic = posts.some(p => 
      p.frontmatter.tags?.some((tag: string) => tag.includes(topic)) ||
      p.frontmatter.title.toLowerCase().includes(topic)
    );
...
function generateIdeas(posts: Post[], count: number, typeFilter?: string): ContentIdea[] {
  ...
  const generators: Record<string, () => ContentIdea | null> = {
    sequel: () => {
      ...
      return {
        ...
        suggestedTags: [...(post.frontmatter.tags || []), 'implementation', 'tutorial'],
        estimatedWords: 1500,
        difficulty: 'medium'
      };
    },
    ...
    'deep-dive': () => {
      ...
      const relatedPosts = posts.filter(p => p.frontmatter.tags?.includes(tag));
    ...
    'counter-point': () => {
      const post = getRandom(posts.filter(p => p.frontmatter.title.includes('Why') || p.frontmatter.title.includes('How')));
      if (!post) return null;
      ...
      return {
        ...
        type: 'counter-point',
        basedOn: post.frontmatter.title,
        rationale: 'Balanced perspectives build trust. This explores the nuances and edge cases.',
        suggestedTags: [...(post.frontmatter.tags || []), 'perspective', 'analysis'],
        estimatedWords: 1200,
        difficulty: 'medium'
      };
    },
    ...
    'case-study': () => {
      const topics = ['scaling', 'hiring', 'product-market-fit', 'pricing', 'architecture'];
      const topic = getRandom(topics);
      
      return {
        title: `From 0 to 100k MRR: A ${topic.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} Case Study`,
        type: 'case-study',
        rationale: 'Real-world examples and data resonate strongly with your audience.',
    ...
    comparison: () => {
      ...
      return {
        ...
        type: 'comparison',
        rationale: 'Comparison posts are highly searched and help readers make informed decisions.',
        suggestedTags: ['comparison', 'tools', 'decision-making', first.toLowerCase(), second.toLowerCase()],
        estimatedWords: 2500,
        difficulty: 'hard'
      };
    }
  ...
  const interactiveIdeas = [
    {
      ...
      rationale: 'Combine your expertise in SaaS metrics with interactive components for maximum engagement.',
      suggestedTags: ['saas', 'metrics', 'interactive', 'financial-modeling'],
      estimatedWords: 1500,
      difficulty: 'medium' as const
    },
    {
      title: 'The Startup Decision Tree: Navigate Critical Choices Interactively',
      type: 'practical-guide' as const,
      rationale: 'Help founders make better decisions with an interactive decision tree component.',
...
function formatTable(ideas: ContentIdea[]): void {
  ...
  ideas.forEach((idea, index) => {
    ...
    console.log(`   ${chalk.gray('Type:')} ${typeColors[idea.type](idea.type)} | ${chalk.gray('Difficulty:')} ${difficultyColors[idea.difficulty](idea.difficulty)} | ${chalk.gray('Est. Words:')} ${idea.estimatedWords}`);
  ...
  if (easyOnes.length > 0) {
    ...
    easyOnes.slice(0, 3).forEach(idea => {
      console.log(`  • "${chalk.green(idea.title)}" (${idea.estimatedWords} words)`);
    });
  }
}
...
function formatMarkdown(ideas: ContentIdea[]): void {
  ...
  Object.entries(byType).forEach(([type, typeIdeas]) => {
    console.log(`## ${type.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}\n`);
