...
function analyzeDraft(draft: DraftPost): DraftAnalysis {
  ...
  if (frontmatter.tags && frontmatter.tags.length > 0) score += 10
  else missingElements.push('tags')
  if (frontmatter.pubDate) score += 5

  // Content checks
  const hasIntro = content.includes('##') || content.length > 100
  if (hasIntro) score += 15
  else suggestions.push('Add introduction paragraph')

  const hasSections = (content.match(/##/g) || []).length >= 2
  if (hasSections) score += 15
  else suggestions.push('Add more sections with ## headers')
  ...
  else suggestions.push('Add conclusion or summary')
  ...
  const draftDate = frontmatter.pubDate
    ? new Date(frontmatter.pubDate)
    : new Date()
  const daysSinceDraft = Math.floor(
    (new Date().getTime() - draftDate.getTime()) / (1000 * 60 * 60 * 24)
  )
...
function formatTable(analyses: DraftAnalysis[]): void {
  ...
  analyses.forEach((analysis, index) => {
    ...
    const scoreColor =
      analysis.completionScore >= 70
        ? chalk.green
        : analysis.completionScore >= 40
          ? chalk.yellow
          : chalk.red
    ...
    console.log(
      `   ${chalk.gray('Score:')} ${scoreColor(analysis.completionScore + '%')} | ${chalk.gray('Words:')} ${analysis.wordCount} | ${chalk.gray('Age:')} ${analysis.daysSinceDraft} days`
    )
    console.log(
      `   ${chalk.gray('Priority:')} ${priorityColor(analysis.priority.toUpperCase())} | ${chalk.gray('Est. Time:')} ${analysis.estimatedTimeToComplete}`
    )

    if (analysis.missingElements.length > 0) {
      console.log(
        `   ${chalk.red('Missing:')} ${analysis.missingElements.join(', ')}`
      )
    }
  ...
  const avgScore = Math.round(
    analyses.reduce((sum, a) => sum + a.completionScore, 0) / analyses.length
  )
  ...
  console.log(
    `  Total words in drafts: ${chalk.green(totalWords.toLocaleString())}`
  )
  ...
  console.log(
    `  1. Finish "${chalk.cyan(topDraft.title)}" - it's ${topDraft.completionScore}% complete (${topDraft.estimatedTimeToComplete})`
  )

  const oldestDraft = [...analyses].sort(
    (a, b) => b.daysSinceDraft - a.daysSinceDraft
  )[0]
  if (oldestDraft.slug !== topDraft.slug) {
    console.log(
      `  2. Consider archiving "${chalk.gray(oldestDraft.title)}" - it's ${oldestDraft.daysSinceDraft} days old`
    )
  }

  const quickWins = analyses.filter(
    (a) => a.completionScore >= 70 && a.wordCount >= 500
  )
  if (quickWins.length > 0) {
    console.log(
      `  3. Quick wins: ${quickWins.length} drafts are >70% complete and could ship today`
    )
  }
}
...
function formatMarkdown(analyses: DraftAnalysis[]): void {
  ...
  console.log(
    `- **Average completion**: ${Math.round(analyses.reduce((sum, a) => sum + a.completionScore, 0) / analyses.length)}%`
  )
  console.log(
    `- **Total words**: ${analyses.reduce((sum, a) => sum + a.wordCount, 0).toLocaleString()}\n`
  )
