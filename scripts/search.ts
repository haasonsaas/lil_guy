...
if (!query) {
  ...
  console.error('Usage: bun run search "query" [-c|--content] [-t|--tags] [--title] [-l|--limit N] [-C|--case]');
  console.error('\nOptions:');
  console.error('  -c, --content  Search in post content (default: searches everything)');
...
async function searchFile(filepath: string, query: string, options: any): Promise<SearchResult | null> {
  try {
    ...
    const searchRegex = new RegExp(query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), options.case ? 'g' : 'gi');
    ...
    // Search in tags
    if ((options.tags || (!options.content && !options.title)) && frontmatter.tags) {
        ...
        if (searchRegex.test(line)) {
          ...
          if (trimmedLine) {
            matches.push({
              ...
              highlight: highlightMatch(trimmedLine.substring(0, 100), query, options.case) + (trimmedLine.length > 100 ? '...' : '')
            });
          }
        }
      });
...
async function search() {
  ...
  console.log(chalk.blue(`🔍 Searching for "${query}" in ${mdFiles.length} posts...\n`));
  ...
  console.log(chalk.green(`Found ${results.length} matching posts${results.length > limit ? ` (showing top ${limit})` : ''}:\n`));
  ...
  limitedResults.forEach((result, index) => {
    ...
    result.matches.forEach(match => {
      const icon = match.location === 'title' ? '📌' : 
                   match.location === 'tags' ? '🏷️ ' : '📝';
      
      if (match.location === 'content') {
        console.log(chalk.gray(`   ${icon} Line ${match.line}: `) + match.highlight);
      } else {
        console.log(chalk.gray(`   ${icon} ${match.location}: `) + match.highlight);
      }
    });
