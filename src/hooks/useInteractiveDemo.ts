...
interface InteractiveDemoState<TConfig, TResults> {
  ...
  // Actions
  updateConfig: (
    updates: Partial<TConfig> | ((prev: TConfig) => TConfig)
  ) => void
  reset: () => void
...
}: InteractiveDemoConfig<TConfig, TResults>): InteractiveDemoState<
  TConfig,
  TResults
> {
  ...
  const [persistedConfig, setPersistedConfig] = useLocalStorage(
    `demo-${demoId}`,
    defaultConfig
  )
