...
export function useAnalytics() {
  ...
  const trackPostView = useCallback((slug: string, title: string, tags: string[] = []) => {
    analytics.trackPostView(slug, title, tags);
  }, []);

  const trackPostReadComplete = useCallback((slug: string, timeSpent: number) => {
  ...
  const trackTagClick = useCallback((tag: string, context: 'post' | 'sidebar' | 'page') => {
...
export function useReadingProgress(slug: string) {
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      const timeSpent = Math.round((Date.now() - startTime.current) / 1000);
      if (timeSpent > 30) { // Only track if they spent more than 30 seconds
        trackCompletion();
      }
    };
