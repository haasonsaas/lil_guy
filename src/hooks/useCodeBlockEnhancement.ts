...
export function useCodeBlockEnhancement(containerRef: React.RefObject<HTMLElement>) {
  useEffect(() => {
    ...
    codeBlocks.forEach((codeElement) => {
      ...
      buttonContainer.className = 'absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10';
      
      // Create copy button
      const copyButton = document.createElement('button');
      copyButton.className = 'code-copy-button inline-flex items-center justify-center gap-1 h-8 px-2 text-xs font-medium border border-border/50 bg-background/80 backdrop-blur-sm hover:bg-background/90 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2';
