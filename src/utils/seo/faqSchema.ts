 ...
 */
export function generateFAQStructuredData(faqItems: FAQItem[]): FAQStructuredData {
  return {
...
export function extractFAQFromContent(content: string): FAQItem[] {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    if (isHeading(currentLine) && isQuestionLike(currentLine)) {
      ...
      while (j < lines.length && !isHeading(lines[j]) && !isQuestionLine(lines[j])) {
        if (lines[j].length > 0) {
          answer += lines[j] + ' ';
        }
        j++;
...
function isQuestionLike(line: string): boolean {
  ...
  return cleanLine.endsWith('?') || 
         /^(how|what|why|when|where|who|which|can|should|would|could|is|are|do|does|will)/i.test(cleanLine);
}
 ...
 */
export function generateCommonFAQs(frontmatter: { title: string; content?: { split: (separator: string) => string[] } }, tags: string[]): FAQItem[] {
  const faqs: FAQItem[] = [];
  ...
  if (tags.includes('ai')) {
    faqs.push({
      ...
      answer: 'AI implementation requires careful planning, proper data preparation, and realistic expectations about capabilities and limitations.'
    });
  }
  ...
  if (tags.includes('startup')) {
    faqs.push({
      ...
      answer: 'Successful startups focus on solving real problems, finding product-market fit, building strong teams, and managing cash flow effectively.'
    });
  }
  ...
  if (tags.includes('product-management')) {
    faqs.push({
      ...
      answer: 'Product managers need strong analytical skills, customer empathy, technical understanding, communication abilities, and strategic thinking.'
    });
  }
  ...
  faqs.push({
    ...
    answer: 'Jonathan Haas is a product and engineering leader with over a decade of experience in enterprise software, AI, and vertical SaaS. He has founded multiple companies and advises startups on product strategy.'
  });
 ...
 */
export function validateFAQItems(items: FAQItem[]): { isValid: boolean; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
  const warnings: string[] = [];
  
  if (items.length === 0) {
    warnings.push('No FAQ items found - consider adding some Q&A content for better SEO');
    return { isValid: true, errors, warnings };
  }
  
  if (items.length > 10) {
    warnings.push('More than 10 FAQ items - consider grouping or prioritizing the most important ones');
  }
  
  items.forEach((item, index) => {
    if (item.question.length < 10) {
      errors.push(`FAQ ${index + 1}: Question too short (minimum 10 characters)`);
    }
    
    if (item.question.length > 200) {
      warnings.push(`FAQ ${index + 1}: Question very long (over 200 characters) - consider shortening`);
    }
    ...
    if (item.answer.length > 1000) {
      warnings.push(`FAQ ${index + 1}: Answer very long (over 1000 characters) - consider shortening`);
    }
