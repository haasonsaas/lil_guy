 ...
 */
function getTopicalCategory(tags: string[]): Array<{ name: string; url: string }> {
  const categories: Array<{ name: string; url: string }> = [];
  const baseUrl = 'https://haasonsaas.com';
  
  // Define topic hierarchies
  const topicMappings: Record<string, { name: string; path: string; parent?: string }> = {
    'ai': { name: 'AI & Machine Learning', path: '/topics/ai' },
    'startup': { name: 'Startup Strategy', path: '/topics/startup' },
    'product-management': { name: 'Product Management', path: '/topics/product' },
    'engineering': { name: 'Engineering Leadership', path: '/topics/engineering' },
...
export function validateBreadcrumbs(breadcrumbs: BreadcrumbItem[]): { 
  ...
  if (breadcrumbs.length < 2) {
    errors.push('Breadcrumbs should have at least 2 items (Home + current page)');
  }

  if (breadcrumbs.length > 6) {
    warnings.push('Breadcrumbs are quite deep (>6 levels) - consider simplifying navigation structure');
  }

  // Check first item is always "Home"
  if (breadcrumbs.length > 0 && breadcrumbs[0].name !== 'Home') {
    warnings.push('First breadcrumb should typically be "Home" for best user experience');
  }

  // Check for proper URL structure
  breadcrumbs.forEach((item, index) => {
    if (!item.url.startsWith('http')) {
      errors.push(`Breadcrumb ${index + 1}: URL should be absolute (include https://)`);
    }
    ...
    if (item.name.length > 60) {
      warnings.push(`Breadcrumb ${index + 1}: Name is quite long (>${60} characters) - consider shortening`);
    }

    // Check position numbering
    const expectedPosition = index + 1;
    if (item.position && item.position !== expectedPosition) {
      warnings.push(`Breadcrumb ${index + 1}: Position should be ${expectedPosition} but is ${item.position}`);
    }
  });
