...
export default function GenerativeArtPage() {
  ...
  const generateLSystemArt = useCallback(
  ...
  const generateFractalTree = useCallback(
    (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
      ctx.fillStyle = colorSchemes[colorScheme].colors[0]
      ctx.fillRect(0, 0, canvas.width, canvas.height)
      ...
      const drawBranch = (
        ...
        drawBranch(
          ...
          endY,
          newLength,
          branchAngle - (angle[0] * Math.PI) / 180 + angleVariation,
          depth - 1
        )
        drawBranch(
          endX,
      ...
      for (const particle of particleSystemRef.current) {
        ...
        for (const other of particleSystemRef.current) {
          ...
          if (dist < scale[0] / 2) {
            ...
            ctx.strokeStyle =
              particle.color +
              Math.floor(connectionAlpha * 255)
                .toString(16)
                .padStart(2, '0')
            ctx.lineWidth = 1
            ctx.beginPath()
