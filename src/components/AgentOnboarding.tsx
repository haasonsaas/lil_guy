...
import { Copy, Check, Play, ChevronRight, Zap, Search, Calculator, FileText } from 'lucide-react';
...
const onboardingSteps: Step[] = [
  ...
  },
  {
    ...
    expected: { message: 'Interactive calculators embedded in relevant blog posts' }
  }
];
...
export default function AgentOnboarding() {
  ...
  const runDemo = async (step: Step) => {
    ...
    try {
      ...
      setDemoResults(prev => ({ 
        ...prev, 
        [step.id]: { error: 'Failed to fetch data. This is expected in development mode.' }
      }));
    } finally {
      ...
      <Card className="border-purple-200 bg-gradient-to-r from-purple-50 to-blue-50">
        <CardHeader>
          <div className="flex items-center space-x-2">
            ...
            <CardTitle className="text-xl text-purple-900">AI Agent Quick Start</CardTitle>
          </div>
          <CardDescription className="text-purple-700">
            Learn how to programmatically access content, tools, and recommendations
          </CardDescription>
          <div className="mt-4">
            <div className="flex justify-between text-sm text-purple-600 mb-2">
              <span>Progress</span>
              <span>{completedSteps.length}/{onboardingSteps.length} completed</span>
            </div>
            ...
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant={isCompleted ? 'default' : 'outline'}>
                    {step.action === 'demo' ? 'Interactive' : step.action === 'copy' ? 'Copy' : 'Explore'}
                  </Badge>
                </div>
              </CardHeader>
              ...
              {canAccess && (
                <CardContent className="space-y-4">
                    ...
                    <div className="bg-gray-900 text-green-400 p-3 rounded-lg font-mono text-sm">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => copyToClipboard(step.example!, step.id)}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {step.action === 'demo' && (
                      ...
                      >
                        ...
                        <span>{loading[step.id] ? 'Running...' : 'Try It Live'}</span>
                      </Button>
                    )}
                  ...
                  {demoResults[step.id] && (
                    <div className="mt-4">
                      <h4 className="text-sm font-medium text-gray-700 mb-2">Live Response:</h4>
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-700 mb-4">
              You're now ready to build AI agents that can effectively interact with this site.
            </p>
