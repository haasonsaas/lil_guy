...
export default function CustomerDevelopmentSimulator() {
  ...
  const calculateInterviewScore = (): number => {
    ...
    const avgEffectiveness =
      currentQuestions.reduce((sum, q) => sum + q.effectiveness, 0) /
      currentQuestions.length
    const biasDeduction =
      currentQuestions.filter((q) => q.bias === 'leading').length * 10
    const assumptionDeduction =
      currentQuestions.filter((q) => q.bias === 'assumption-heavy').length * 5

    const baseScore = (avgEffectiveness / 10) * 100
    const finalScore = Math.max(
      0,
      baseScore - biasDeduction - assumptionDeduction
    )
  ...
  const generateInsights = (): string[] => {
    ...
    const openQuestions = currentQuestions.filter(
      (q) => q.type === 'open' || q.type === 'workflow'
    )
    const validationQuestions = currentQuestions.filter(
      (q) => q.type === 'validation'
    )

    if (openQuestions.length >= 3) {
      insights.push(
        "Good job focusing on open-ended questions to understand the customer's world"
      )
    }

    if (validationQuestions.length >= 2) {
      insights.push(
        "Strong validation approach - you're testing assumptions with specific examples"
      )
    }

    if (currentQuestions.some((q) => q.type === 'workflow')) {
      insights.push(
        'Excellent focus on understanding current workflows and processes'
      )
    }

    const highQualityQuestions = currentQuestions.filter(
      (q) => q.effectiveness >= 7
    )
    if (highQualityQuestions.length >= currentQuestions.length * 0.7) {
      insights.push(
        'Most of your questions were well-crafted and likely to yield valuable insights'
      )
    }
  ...
  const identifyRedFlags = (): string[] => {
    ...
    const leadingQuestions = currentQuestions.filter(
      (q) => q.bias === 'leading'
    )
    if (leadingQuestions.length > 2) {
      redFlags.push(
        'Too many leading questions - you may be confirming bias rather than discovering truth'
      )
    }

    const solutionQuestions = currentQuestions.filter(
      (q) => q.type === 'solution'
    )
    if (
      solutionQuestions.length > 1 &&
      currentQuestions.filter((q) => q.type === 'problem').length < 2
    ) {
      redFlags.push(
    ...
    if (
      currentQuestions.filter((q) => q.type === 'pricing').length > 0 &&
      currentQuestions.filter((q) => q.type === 'validation').length < 2
    ) {
    ...
    if (timeElapsed > 1800) {
      // 30 minutes
      redFlags.push(
        "Interview running long - respect the customer's time constraints"
      )
    }
    ...
    <div className="w-full max-w-6xl mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Practice customer interview techniques with realistic personas.
            Learn to ask better questions and avoid common biases.
          </p>
        </CardHeader>
      </Card>
      ...
      <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="interview" disabled={!selectedPersona}>
            Interview
          </TabsTrigger>
                          ...
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-500">
                              ...
                              <span className="capitalize">
                                {persona.skepticismLevel}
                              </span>
                            </span>
                            <span className="text-gray-500">
                              Time:{' '}
                              <span className="capitalize">
                                {persona.timeConstraints.replace('_', ' ')}
                              </span>
                            </span>
                          </div>
                        </div>
              ...
              {selectedPersona && (
                <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                    You're interviewing {selectedPersona.name} to understand
                    their challenges and validate potential solutions. They have
                    a {selectedPersona.communicationStyle} communication style
                    and {selectedPersona.skepticismLevel} skepticism level.
                  </p>
              ...
              <CardContent>
                {currentQuestions.length === 0 ? (
                  <p className="text-gray-500 italic text-center py-8">
                    No questions asked yet. Start by selecting questions from
                    the sample list.
                  </p>
                ) : (
                  <div className="space-y-3">
                ...
                <Card>
                  ...
                  <CardContent>
                    {results.redFlags.length > 0 ? (
                      ...
                      <p className="text-gray-500 italic">
                        No major red flags identified. Good job!
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  ...
                  <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                      <li>
                        • Practice with different personas to build versatility
                      </li>
                      <li>
                      ...
                      <li>
                        • Ask about current workflows before proposing changes
                      </li>
                    </ul>
                  </div>
                </CardContent>
