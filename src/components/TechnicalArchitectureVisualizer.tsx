...
import { Plus, Database, Server, Globe, Smartphone, Shield, X, Settings, Layers, Zap, AlertTriangle } from 'lucide-react';

interface ArchitectureComponent {
  id: string;
  name: string;
  type: 'frontend' | 'backend' | 'database' | 'cache' | 'queue' | 'cdn' | 'auth' | 'monitoring' | 'api' | 'storage';
  technology: string;
  description: string;
    ...
    >
      <Card className="w-32 h-28 border-2" style={{ borderColor: color }}>
        <CardContent className="p-2 text-center">
          ...
          <div className="text-xs font-semibold mb-1 truncate">{component.name}</div>
          <div className="text-xs text-muted-foreground mb-1 truncate">{component.technology}</div>
...
export default function TechnicalArchitectureVisualizer() {
  const [components, setComponents] = useState<ArchitectureComponent[]>(defaultComponents);
  ...
  const [newComponent, setNewComponent] = useState<Partial<ArchitectureComponent>>({
    name: '',
    type: 'backend',
    technology: '',
    description: '',
  ...
  const analyzeArchitecture = useCallback(() => {
    ...
    if (averageScalability < 6) {
      recommendations.push('Consider upgrading components with low scalability scores');
    }
    
    if (totalComplexity / components.length > 7) {
      recommendations.push('High average complexity detected - consider simplifying architecture');
    }
    
    if (singlePointsOfFailure.length > 0) {
      recommendations.push(`Single points of failure detected: ${singlePointsOfFailure.join(', ')}`);
    }
    
    if (technologyDiversity > components.length * 0.7) {
      recommendations.push('High technology diversity may increase maintenance burden');
    }
    
    if (!components.some(comp => comp.type === 'monitoring')) {
      recommendations.push('Consider adding monitoring and observability components');
    }
  ...
  const selectedComp = selectedComponent ? components.find(comp => comp.id === selectedComponent) : null;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold mb-2">Technical Architecture Visualizer</h2>
        <p className="text-muted-foreground">
          Design and analyze your technical architecture. Visualize components, connections, and complexity.
        </p>
      </div>
          ...
          <Card>
            ...
            <CardContent>
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none">
                  {components.map(component => 
                    component.connections.map(connId => {
                      const targetComp = components.find(comp => comp.id === connId);
              ...
              <div className="mt-4 text-xs text-muted-foreground">
                Drag components to reposition them. Click to select and view details.
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          {selectedComp && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    {React.createElement(componentTypes[selectedComp.type].icon, { className: "h-5 w-5" })}
                    {selectedComp.name}
                  </span>
                  <Button
                    variant="ghost"
              ...
              <CardContent>
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">{selectedComp.technology}</div>
                  </div>
                  
                  <div>
                    <div className="font-semibold text-sm">Description</div>
                    <div className="text-sm text-muted-foreground">{selectedComp.description}</div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <span className={`ml-1 font-medium ${getScoreColor(selectedComp.scalability)}`}>
                        {selectedComp.scalability}/10
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Complexity:</span>
                      <span className={`ml-1 font-medium ${getScoreColor(selectedComp.complexity, true)}`}>
                        {selectedComp.complexity}/10
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Maintenance:</span>
                      <span className={`ml-1 font-medium ${getScoreColor(selectedComp.maintenance, true)}`}>
                        {selectedComp.maintenance}/10
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Cost:</span>
                      <span className={`ml-1 font-medium ${getScoreColor(selectedComp.cost, true)}`}>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <span className="ml-1 font-medium">{components.length}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Technologies:</span>
                    <span className="ml-1 font-medium">{analysis.technologyDiversity}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Avg Scalability:</span>
                    <span className={`ml-1 font-medium ${getScoreColor(analysis.averageScalability)}`}>
                      {analysis.averageScalability.toFixed(1)}/10
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Total Complexity:</span>
                    <span className={`ml-1 font-medium ${getScoreColor(analysis.totalComplexity / components.length, true)}`}>
                      {(analysis.totalComplexity / components.length).toFixed(1)}/10
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <div className="font-semibold text-sm mb-2">Component Types</div>
                  <div className="flex flex-wrap gap-1">
                    {Object.entries(componentsByType).map(([type, count]) => (
                      <Badge 
                        key={type} 
                        variant="outline" 
                        style={{ borderColor: componentTypes[type as keyof typeof componentTypes].color }}
                      >
                        {componentTypes[type as keyof typeof componentTypes].label}: {count}
                      </Badge>
                    ))}
                  </div>
                </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {analysis.recommendations.map((rec, index) => (
                    <div key={index} className="p-2 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded text-xs text-blue-800 dark:text-blue-300">
                      {rec}
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewComponent(prev => ({...prev, type: value as ArchitectureComponent['type']}))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-2 mt-2">
                    {['scalability', 'complexity', 'maintenance', 'cost', 'maturity'].map(field => (
                      <div key={field} className="flex items-center gap-2">
                        <Label htmlFor={field} className="w-20 text-xs capitalize">{field}</Label>
                        <Input
                          id={field}
                          type="number"
                          min="1"
                          max="10"
                          value={newComponent[field as keyof typeof newComponent] as number}
            ...
            {analysis.averageScalability < 6 && (
              <div className="p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <p className="text-yellow-800 dark:text-yellow-300">
                  ⚠️ <strong>Scalability concerns:</strong> Average scalability score of {analysis.averageScalability.toFixed(1)} 
                  suggests potential bottlenecks. Consider upgrading low-scoring components.
                </p>
              </div>
            )}
            ...
            {analysis.totalComplexity / components.length > 7 && (
              <div className="p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
                <p className="text-red-800 dark:text-red-300">
                  🚨 <strong>High complexity:</strong> Average complexity of {(analysis.totalComplexity / components.length).toFixed(1)} 
                  may impact maintainability. Consider architectural simplification.
                </p>
              </div>
            )}
            ...
            {!components.some(comp => comp.type === 'monitoring') && (
              <div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-blue-800 dark:text-blue-300">
                  📊 <strong>Missing observability:</strong> No monitoring components detected. 
                  Consider adding logging, metrics, and alerting capabilities.
                </p>
              </div>
            )}
            ...
            {analysis.technologyDiversity > components.length * 0.7 && (
              <div className="p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
                <p className="text-orange-800 dark:text-orange-300">
                  🔧 <strong>Technology sprawl:</strong> {analysis.technologyDiversity} different technologies 
                  for {components.length} components may increase operational complexity.
                </p>
              </div>
            )}
            
            {analysis.averageScalability >= 8 && analysis.totalComplexity / components.length <= 5 && (
              <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg">
                <p className="text-green-800 dark:text-green-300">
                  🚀 <strong>Well-designed architecture:</strong> High scalability with manageable complexity. 
                  Your architecture is positioned for sustainable growth.
                </p>
              </div>
            )}
          </div>
