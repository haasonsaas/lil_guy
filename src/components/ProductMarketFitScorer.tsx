...
import { Target, Users, Heart, TrendingUp, AlertCircle, CheckCircle } from 'lucide-react';
...
const questions: PMFQuestion[] = [
  {
    ...
    options: [
      { value: 100, label: 'Very disappointed', description: 'Strong PMF signal' },
      { value: 60, label: 'Somewhat disappointed', description: 'Moderate PMF' },
  ...
  },
  {
    ...
    options: [
      { value: 100, label: '9-10 (Promoters)', description: 'Love the product' },
      { value: 50, label: '7-8 (Passives)', description: 'Satisfied but not enthusiastic' },
      { value: 0, label: '0-6 (Detractors)', description: 'Unlikely to recommend' }
    ],
    weight: 0.15
  },
  {
    ...
    options: [
      { value: 100, label: 'Daily or multiple times per day', description: 'High engagement' },
      { value: 75, label: 'Weekly', description: 'Good engagement' },
      { value: 40, label: 'Monthly', description: 'Moderate engagement' },
      { value: 10, label: 'Rarely or sporadically', description: 'Low engagement' }
    ],
    weight: 0.20
  },
  {
  ...
  },
  {
    id: 'growth_channel',
    category: 'growth',
    question: 'What percentage of new customers come from word-of-mouth/referrals?',
    options: [
  ...
  },
  {
    ...
    options: [
      { value: 100, label: 'Massive market (>$10B TAM)', description: 'Huge opportunity' },
      { value: 80, label: 'Large market ($1-10B TAM)', description: 'Significant opportunity' },
      { value: 60, label: 'Medium market ($100M-1B TAM)', description: 'Good opportunity' },
      { value: 30, label: 'Small market (<$100M TAM)', description: 'Limited opportunity' }
    ],
    weight: 0.05
  }
];
  ...
  const getInsights = () => {
    ...
    if (scores.overall >= 80) {
      insights.push({
        ...
        message: 'You have strong PMF signals. Focus on scaling growth and maintaining quality.'
      });
    } else if (scores.overall >= 60) {
      insights.push({
        type: 'warning',
        title: 'Moderate Product-Market Fit',
        message: 'You\'re on the right track but need improvements in key areas.'
      });
    } else if (scores.overall >= 40) {
      insights.push({
        type: 'error',
        title: 'Weak Product-Market Fit',
        message: 'Significant changes needed. Consider pivoting or major product improvements.'
      });
    } else {
      insights.push({
        type: 'error',
        title: 'No Product-Market Fit',
        message: 'Major product changes or pivot required. Focus on customer development.'
      });
    }
    ...
    if (scores.satisfaction < 50) {
      insights.push({
        ...
        message: 'Focus on understanding customer needs and improving core value proposition.'
      });
    }
    ...
    if (scores.retention < 50) {
      insights.push({
        ...
        message: 'Users aren\'t sticking around. Improve onboarding and core product experience.'
      });
    }
    ...
    if (scores.growth < 50) {
      insights.push({
        ...
        message: 'Low word-of-mouth growth suggests product isn\'t compelling enough to share.'
      });
    }
    ...
    <div className="w-full max-w-4xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          Assess your product-market fit across key dimensions. Based on proven PMF frameworks.
        </p>
      </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center mb-6">
                <div className={`text-6xl font-bold mb-2 ${getScoreColor(scores.overall)}`}>
                  {Math.round(scores.overall)}
                </div>
                <Badge className={`text-lg py-1 px-3 ${getScoreBadgeColor(scores.overall)}`}>
                ...
                {scores.overall >= 70 && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg">
                    <p className="text-blue-800 dark:text-blue-300">
                      🚀 <strong>Scale with confidence:</strong> Your PMF signals are strong enough to invest in growth channels and team expansion.
                    </p>
                  </div>
                )}
                ...
                {scores.overall >= 50 && scores.overall < 70 && (
                  <div className="p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    <p className="text-yellow-800 dark:text-yellow-300">
                      🔧 <strong>Optimize core metrics:</strong> Focus on improving weak areas before scaling. Run more customer interviews.
                    </p>
                  </div>
                )}
                ...
                {scores.overall < 50 && (
                  <div className="p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
                    <p className="text-red-800 dark:text-red-300">
                      ⚠️ <strong>Back to customer development:</strong> Don't scale yet. Deeply understand customer problems and iterate on core value prop.
                    </p>
                  </div>
                )}
              </div>
