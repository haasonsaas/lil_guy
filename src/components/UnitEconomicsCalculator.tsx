...
function calculateUnitEconomics(metrics: Metrics): Results {
  ...
  const ltv =
    monthlyChurnRate > 0 ? (arpu * grossMarginDecimal) / monthlyChurnRate : 0
  const ltvCacRatio = cac > 0 ? ltv / cac : 0
  const paybackPeriod =
    arpu * grossMarginDecimal > 0 ? cac / (arpu * grossMarginDecimal) : 0
  ...
  if (ltvCacRatio >= 3) {
    isHealthy = true
    recommendation =
      '🟢 Excellent! Your unit economics are strong and sustainable.'
  } else if (ltvCacRatio >= 2) {
    isHealthy = true
    recommendation =
      '🟡 Good foundation, but room for improvement in efficiency.'
  } else if (ltvCacRatio >= 1) {
    recommendation =
      "🟠 Concerning - you're barely profitable. Focus on retention or reduce CAC."
  } else {
    recommendation =
      "🔴 Critical - you're losing money on every customer. Immediate action needed."
  }
...
function validateMetrics(metrics: Metrics): {
  isValid: boolean
  errors?: string[]
} {
  const errors: string[] = []
  ...
  if (metrics.churnRate < 0 || metrics.churnRate > 100)
    errors.push('Churn rate must be between 0-100%')
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange('churnRate', e.target.value)
                  }
                  placeholder="5"
                />
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-lg border bg-gradient-to-r from-primary/10 to-background">
                  ...
                  <div className="text-sm">
                    {results.recommendation || 'Enter values to see assessment'}
                  </div>
                </div>
              </div>
