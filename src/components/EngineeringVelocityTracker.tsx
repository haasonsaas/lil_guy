...
import { TrendingUp, TrendingDown, AlertTriangle, CheckCircle, Code, Clock, Target, Zap, RefreshCw, AlertCircle } from 'lucide-react';

interface TechnicalDebtItem {
  id: string;
  category: 'architecture' | 'code_quality' | 'documentation' | 'testing' | 'infrastructure';
...
export default function EngineeringVelocityTracker() {
  ...
  const velocityProjection = useMemo(() => {
    ...
    const totalHoursPerSprint = teamMetrics.teamSize * teamMetrics.sprintLength * 40; // 40 hours per week
    const effectiveHours = totalHoursPerSprint * (1 - teamMetrics.meetingOverhead / 100) * (1 - teamMetrics.bugFixTime / 100);
    ...
    for (let sprint = 0; sprint <= timeHorizon; sprint++) {
      ...
      const currentDebtImpact = allDebtItems.reduce((total, item) => {
        ...
        const growthFactor = 1 + (item.accumulationRate / 100) * sprintsSinceStart;
      ...
      const netDebtImpact = Math.max(0, currentDebtImpact - paydownEffect * sprint);
      
      cumulativeDebtImpact = netDebtImpact;
      
      const actualVelocity = Math.max(0, baseVelocity * (1 - Math.min(0.8, cumulativeDebtImpact / 100)));
      const velocityLoss = baseVelocity - actualVelocity;
      
      // Time to market calculation (cumulative feature delivery)
      const cumulativeFeatures = sprint === 0 ? 0 : data[sprint - 1].cumulative + actualVelocity;
  ...
  const insights = useMemo(() => {
    ...
    const velocityLoss = ((initialVelocity - finalVelocity) / initialVelocity) * 100;
    
    const totalDebtEffort = allDebtItems.reduce((sum, item) => sum + item.effortToFix, 0);
    const sprintsToPayDebt = Math.ceil(totalDebtEffort / (teamMetrics.teamSize * teamMetrics.sprintLength * 40 * (debtPaydownRate / 100)));
    
    const criticalDebt = allDebtItems.filter(item => item.severity === 'critical');
    ...
    return {
      ...
      worstCategory: allDebtItems.reduce((worst, item) => 
        item.velocityImpact > worst.velocityImpact ? item : worst, allDebtItems[0])?.category || 'none'
    };
  ...
  const updateCustomDebtItem = (id: string, updates: Partial<TechnicalDebtItem>) => {
    setCustomDebtItems(customDebtItems.map(item => 
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'architecture': return <Target className="w-4 h-4" />;
      case 'code_quality': return <Code className="w-4 h-4" />;
      case 'testing': return <CheckCircle className="w-4 h-4" />;
      case 'documentation': return <AlertCircle className="w-4 h-4" />;
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Model how technical debt impacts your team's delivery velocity over time. 
            Understand the compound effects and plan debt paydown strategies.
          </p>
        </CardHeader>
      </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTeamMetrics({...teamMetrics, teamSize: parseInt(e.target.value) || 1})}
                    min="1"
                    max="20"
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTeamMetrics({...teamMetrics, sprintLength: parseInt(e.target.value) || 1})}
                    min="1"
                    max="4"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTeamMetrics({...teamMetrics, storiesPerSprint: parseInt(e.target.value) || 1})}
                    min="1"
                    max="50"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTeamMetrics({...teamMetrics, avgStoryPoints: parseInt(e.target.value) || 1})}
                    min="1"
                    max="13"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="hoursPerStoryPoint">Hours per Story Point</Label>
                  <Input
                    id="hoursPerStoryPoint"
                    type="number"
                    value={teamMetrics.hoursPerStoryPoint}
                    onChange={(e) => setTeamMetrics({...teamMetrics, hoursPerStoryPoint: parseInt(e.target.value) || 1})}
                    min="1"
                    max="20"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="codeReviewTime">Code Review Time (hrs/story)</Label>
                  <Input
                    id="codeReviewTime"
                    type="number"
                    value={teamMetrics.codeReviewTime}
                    onChange={(e) => setTeamMetrics({...teamMetrics, codeReviewTime: parseInt(e.target.value) || 0})}
                    min="0"
                    max="10"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Slider
                    ...
                    onValueChange={(value) => setTeamMetrics({...teamMetrics, bugFixTime: value[0]})}
                    max={50}
                    step={5}
                    className="w-full"
                  />
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>0%</span>
                    <span className="font-medium">{teamMetrics.bugFixTime}%</span>
                    <span>50%</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <Slider
                    ...
                    onValueChange={(value) => setTeamMetrics({...teamMetrics, meetingOverhead: value[0]})}
                    max={50}
                    step={5}
                    className="w-full"
                  />
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>0%</span>
                    <span className="font-medium">{teamMetrics.meetingOverhead}%</span>
                    <span>50%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <h4 className="font-medium text-blue-900 dark:text-blue-100 mb-2">Calculated Baseline</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    <div className="text-blue-800 dark:text-blue-200">Total Hours/Sprint</div>
                    <div className="font-bold">{teamMetrics.teamSize * teamMetrics.sprintLength * 40}</div>
                  </div>
                  <div>
                    <div className="text-blue-800 dark:text-blue-200">Effective Hours</div>
                    <div className="font-bold">
                      {Math.round(teamMetrics.teamSize * teamMetrics.sprintLength * 40 * 
                        (1 - teamMetrics.meetingOverhead / 100) * 
                        (1 - teamMetrics.bugFixTime / 100))}
                    </div>
                  </div>
                  <div>
                    <div className="text-blue-800 dark:text-blue-200">Base Velocity</div>
                    <div className="font-bold">
                      {Math.round((teamMetrics.teamSize * teamMetrics.sprintLength * 40 * 
                        (1 - teamMetrics.meetingOverhead / 100) * 
                        (1 - teamMetrics.bugFixTime / 100)) / teamMetrics.hoursPerStoryPoint)} SP
                    </div>
                  </div>
                  <div>
                    <div className="text-blue-800 dark:text-blue-200">Stories/Sprint</div>
                    <div className="font-bold">
                      {Math.round(((teamMetrics.teamSize * teamMetrics.sprintLength * 40 * 
                        (1 - teamMetrics.meetingOverhead / 100) * 
                        (1 - teamMetrics.bugFixTime / 100)) / teamMetrics.hoursPerStoryPoint) / teamMetrics.avgStoryPoints)}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="space-y-6">
                ...
                <Select value={selectedScenario} onValueChange={setSelectedScenario}>
                  ...
                  <SelectContent>
                    {DEBT_SCENARIOS.map((scenario) => (
                      <SelectItem 
                        key={scenario.name.toLowerCase().replace(' ', '_').replace('-', '_')} 
                        value={scenario.name.toLowerCase().replace(' ', '_').replace('-', '_')}
                      >
                        {scenario.name}
                      </SelectItem>
                    ))}
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button onClick={addCustomDebtItem} variant="outline" size="sm">
                    Add Custom Item
                  </Button>
                </div>
                        ...
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1 space-y-2">
                            ...
                            {item.id.startsWith('custom_') ? (
                              <div className="space-y-2">
                                <Input
                                  ...
                                  onChange={(e) => updateCustomDebtItem(item.id, { description: e.target.value })}
                                />
                                <div className="grid grid-cols-3 gap-2">
                                  <Input
                                    ...
                                    onChange={(e) => updateCustomDebtItem(item.id, { effortToFix: parseInt(e.target.value) || 0 })}
                                  />
                                  <Input
                                    type="number"
                                    placeholder="Impact (%)"
                                    value={item.velocityImpact}
                                    onChange={(e) => updateCustomDebtItem(item.id, { velocityImpact: parseInt(e.target.value) || 0 })}
                                  />
                                  <Input
                                    type="number"
                                    placeholder="Growth rate"
                                    value={item.accumulationRate}
                                    onChange={(e) => updateCustomDebtItem(item.id, { accumulationRate: parseFloat(e.target.value) || 0 })}
                                  />
                                </div>
                              </div>
                            ) : (
                            ...
                            <div className="flex gap-4 text-xs text-gray-500">
                              ...
                              <span>Growth: {item.accumulationRate}x/sprint</span>
                            </div>
                          </div>
                        </div>
              ...
              <CardHeader>
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    className="w-48"
                  />
                  <div className="text-sm text-gray-500">{timeHorizon} sprints</div>
                </div>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={velocityProjection}>
                    ...
                    <XAxis 
                      dataKey="sprint" 
                      label={{ value: 'Sprint', position: 'insideBottom', offset: -10 }}
                    />
                    <YAxis 
                      label={{ value: 'Velocity (Story Points)', angle: -90, position: 'insideLeft' }}
                    />
                    <Tooltip 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {Math.round((debtPaydownRate / 100) * teamMetrics.teamSize * teamMetrics.sprintLength * 40)}h
                  </div>
                  <div className="text-sm text-gray-600 dark:text-gray-300">
                    Hours per Sprint
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {insights.criticalIssues > 0 && (
                    <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                      ...
                      <p className="text-sm text-red-800 dark:text-red-200">
                        You have {insights.criticalIssues} critical technical debt items. 
                        Consider allocating 30-40% of capacity to debt reduction immediately.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2 text-sm">
                      <li className="flex items-start gap-2">
                        ...
                        <span>Start with highest impact, lowest effort items for quick wins</span>
                      </li>
                      <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                        <span>Address {insights.worstCategory.replace('_', ' ')} issues first as they have the highest velocity impact</span>
                      </li>
                      <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                        <span>Allocate {Math.max(20, Math.min(40, insights.criticalIssues * 10))}% capacity to debt paydown</span>
                      </li>
                      <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                        <span>Track velocity improvements as debt is reduced</span>
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="pt-4 border-t">
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      <strong>Pro tip:</strong> Technical debt compounds over time. The cost of fixing it later 
                      is always higher than addressing it now. Use this model to make data-driven decisions 
                      about when to prioritize debt reduction vs. new features.
                    </p>
                  </div>
                </CardContent>
              </Card>
