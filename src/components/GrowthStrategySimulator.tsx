...
import { TrendingUp, Target, DollarSign, Users, Zap, AlertTriangle } from 'lucide-react';
      ...
      <CardContent className="p-4">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-muted-foreground">CAC:</span> {formatCurrency(channel.cac)}
            </div>
            <div>
              <span className="text-muted-foreground">LTV:</span> {formatCurrency(channel.ltv)}
            </div>
            <div>
              <span className="text-muted-foreground">Conv Rate:</span> {channel.conversionRate}%
            </div>
            <div>
              <span className="text-muted-foreground">Max/Month:</span> {channel.maxCapacity}
            </div>
          </div>
          
          <div className="space-y-2">
            <Label htmlFor={`allocation-${channel.id}`}>Budget Allocation (%)</Label>
            <Input
              ...
              max="100"
              value={allocation}
              onChange={(e) => onAllocationChange(channel.id, parseFloat(e.target.value) || 0)}
            />
...
export default function GrowthStrategySimulator() {
  ...
  const runSimulation = () => {
    ...
    for (let month = 1; month <= strategy.timeHorizon; month++) {
      ...
      let monthlySpend = 0;
      const channelBreakdown: Record<string, { customers: number; spend: number; revenue: number }> = {};
      ...
      strategy.channels.forEach(channel => {
        ...
        const customersAcquired = Math.min(maxCustomersFromBudget, effectiveCapacity);
      ...
      const currentMRR = strategy.currentMRR + (totalCustomers - (strategy.currentMRR / strategy.averagePrice)) * strategy.averagePrice;
  ...
  const totalAllocation = Object.values(channelAllocations).reduce((sum, val) => sum + val, 0);
  const finalResult = results[results.length - 1];
  const reachesTarget = finalResult ? finalResult.mrr >= strategy.targetRevenue / 12 : false;
  ...
  const channelContributionData = strategy.channels.map(channel => {
    const allocation = channelAllocations[channel.id] || 0;
    const monthlyBudget = (strategy.totalBudget / strategy.timeHorizon * allocation) / 100;
    const projectedCustomers = (monthlyBudget / channel.cac) * strategy.timeHorizon * 0.7; // Average ramp factor
    ...
    <div className="w-full max-w-7xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          Model different growth channels and budget allocations. See the impact on revenue and ROI.
        </p>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateStrategy('totalBudget', parseFloat(e.target.value) || 0)}
                placeholder="500000"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateStrategy('timeHorizon', parseFloat(e.target.value) || 12)}
                placeholder="24"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateStrategy('currentMRR', parseFloat(e.target.value) || 0)}
                placeholder="50000"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateStrategy('averagePrice', parseFloat(e.target.value) || 100)}
                placeholder="199"
              />
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-muted rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <Badge className={totalAllocation === 100 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300'}>
                {totalAllocation}%
              </Badge>
            </div>
            {totalAllocation !== 100 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Final MRR</p>
                  <p className="text-2xl font-bold">{formatCurrency(finalResult.mrr)}</p>
                  <Badge className={reachesTarget ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'}>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatNumber(finalResult.totalCustomers)}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatCurrency(finalResult.totalSpend)}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge className={finalResult.roi > 200 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : finalResult.roi > 100 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'}>
                    {finalResult.roi > 200 ? 'Excellent' : finalResult.roi > 100 ? 'Good' : 'Poor'}
                  </Badge>
                </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="Total Spend" stroke="#ef4444" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
          ...
          <div className="h-64 mb-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={channelContributionData}>
                ...
                <YAxis />
                <Tooltip formatter={(value, name) => [
                  name === 'efficiency' ? `${value}x` : name === 'investment' ? formatCurrency(Number(value)) : formatNumber(Number(value)),
                  name === 'efficiency' ? 'LTV:CAC Ratio' : name === 'investment' ? 'Investment' : 'Customers'
                ]} />
                <Legend />
                <Bar dataKey="customers" fill="#3b82f6" name="Projected Customers" />
            ...
            {totalAllocation < 90 && (
              <div className="p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <p className="text-yellow-800 dark:text-yellow-300">
                  ⚠️ <strong>Underutilized budget:</strong> Only {totalAllocation}% of budget allocated. 
                  Consider increasing investment in high-performing channels.
                </p>
              </div>
            )}
            ...
            {finalResult && finalResult.roi < 100 && (
              <div className="p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
                <p className="text-red-800 dark:text-red-300">
                  🚨 <strong>Negative ROI:</strong> Current strategy projects {finalResult.roi}% ROI. 
                  Focus budget on channels with better LTV:CAC ratios.
                </p>
              </div>
            )}
            ...
            {!reachesTarget && finalResult && (
              <div className="p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
                <p className="text-orange-800 dark:text-orange-300">
                  📊 <strong>Target shortfall:</strong> Projected MRR of {formatCurrency(finalResult.mrr)} falls short of target. 
                  Consider increasing budget or improving channel efficiency.
                </p>
              </div>
            )}
            
            {strategy.channels.some(ch => (channelAllocations[ch.id] || 0) > 40) && (
              <div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-blue-800 dark:text-blue-300">
                  🎯 <strong>Channel concentration risk:</strong> Heavy allocation to single channel increases risk. 
                  Consider diversifying across multiple growth channels.
                </p>
              </div>
            )}
            ...
            {finalResult && finalResult.roi > 300 && (
              <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg">
                <p className="text-green-800 dark:text-green-300">
                  🚀 <strong>Strong strategy:</strong> Excellent ROI of {finalResult.roi}% suggests efficient growth channels. 
