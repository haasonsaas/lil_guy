...
export default function TechnicalDebtSimulator() {
  ...
  const simulateDebt = (params: DebtConfig, name: string): Scenario => {
    ...
    if (name === 'Keep Accumulating') {
      description =
        '🔴 Velocity crashes as debt compounds. Short-term gains, long-term pain.'
    } else if (name === 'Aggressive Refactoring') {
      description =
        '🟢 Slower start but maintains velocity. Sustainable long-term delivery.'
    } else {
      description =
        '🟡 Balanced approach. Some debt management with steady feature delivery.'
    }
  ...
  const getBestScenario = () => {
    ...
    return scenarios.reduce((best, current) =>
      ...
      best.cumulativeFeatures[best.cumulativeFeatures.length - 1]
        ? current
        : best
    )
  }
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-muted-foreground text-center">
            See how technical debt compounds and impacts engineering velocity
            over time
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="velocity">
                  Initial Velocity (features/month)
                </Label>
                <Input
                  id="velocity"
                  type="number"
                  ...
                  onChange={(e) =>
                    handleInputChange('initialVelocity', e.target.value)
                  }
                  placeholder="10"
                />
              </div>
              ...
              <div className="space-y-3">
                <Label>
                  Debt Accumulation Rate: {config.debtAccumulation}%
                </Label>
                <Slider
                  value={[config.debtAccumulation]}
                  onValueChange={(value) =>
                    handleInputChange('debtAccumulation', value[0])
                  }
                  min={0}
                  max={50}
                  step={1}
              ...
              <div className="space-y-3">
                ...
                <Slider
                  ...
                  onValueChange={(value) =>
                    handleInputChange('maintenanceEffort', value[0])
                  }
                  min={0}
                  max={50}
                  step={1}
              ...
              <div className="space-y-3">
                ...
                <Slider
                  ...
                  onValueChange={(value) =>
                    handleInputChange('refactoringEffort', value[0])
                  }
                  min={0}
                  max={50}
                  step={1}
                ...
                >
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      Features:{' '}
                      {
                        scenario.cumulativeFeatures[
                          scenario.cumulativeFeatures.length - 1
                        ]
                      }
                    </div>
                    <div>
                      Final Velocity:{' '}
                      {
                        scenario.velocityOverTime[
                          scenario.velocityOverTime.length - 1
                        ]
                      }
                    </div>
                    <div>
                      Debt Level:{' '}
                      {scenario.debtLevel[scenario.debtLevel.length - 1]}
                    </div>
                  </div>
              ...
              <div className="space-y-2">
                {scenarios.map((scenario, scenarioIndex) => (
                  <div key={scenario.name} className="space-y-1">
                    <div
                      className={`text-sm font-medium ${getScenarioColor(scenarioIndex)}`}
                    >
                      {scenario.name}
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 text-sm">
                <div className="p-3 rounded bg-blue-50 dark:bg-blue-950/20">
                  ...
                  <div className="text-xs mt-1">
                    Technical debt compounds like financial debt, growing
                ...
                <div className="p-3 rounded bg-yellow-50 dark:bg-yellow-950/20">
                  ...
                  <div className="text-xs mt-1">
                    Teams hit a wall where adding features becomes impossibly
                    slow.
                  </div>
                </div>
              ...
              <div className="text-xs text-muted-foreground mt-4">
                ...
                <div>
                  • Early investment in quality pays long-term dividends
                </div>
              </div>
            </div>
          </div>
