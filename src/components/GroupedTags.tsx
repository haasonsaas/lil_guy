...
export default function GroupedTags({ groups, className = '' }: GroupedTagsProps) {
  ...
  return (
    <div className={cn('space-y-6', className)}>
      {groups.map((group) => (
        <div key={group.name} className="border border-border/50 rounded-2xl overflow-hidden bg-card/50 backdrop-blur-sm">
          <button
          ...
          >
            <div className="flex items-center gap-2">
              {group.icon}
              <span className="font-display font-semibold text-lg">{group.name}</span>
            </div>
