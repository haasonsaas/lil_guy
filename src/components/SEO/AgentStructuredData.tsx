...
export function AgentStructuredData({
  ...
  const extractTopics = (text: string, tags: string[] = []) => {
    ...
    const foundTechTerms = techTerms.filter(
      (term) => text.toLowerCase().includes(term) || tags.includes(term)
    )

    const foundBusinessTerms = businessTerms.filter(
      (term) => text.toLowerCase().includes(term) || tags.includes(term)
    )
  ...
  const getComplexityLevel = () => {
    ...
    if (
      frontmatter.tags?.some((tag) =>
        ['tutorial', 'guide', 'how-to'].includes(tag)
      )
    )
      return 'Intermediate'
    return 'Beginner'
  ...
  const agentStructuredData = {
    ...
    isPartOf: {
      ...
      url: `${baseUrl}/blog`,
      description:
        'Expert insights on bridging technical vision with market reality',
    },
  ...
  return (
    <Helmet>
      ...
      <meta
        name="reading-time"
        content={(readingTime || Math.ceil(wordCount / 200)).toString()}
      />
