...
const getHealthScore = (
  metric: string,
  value: number
): { score: 'excellent' | 'good' | 'warning' | 'danger'; message: string } => {
  switch (metric) {
    case 'ltvCacRatio':
      if (value >= 3)
        return { score: 'excellent', message: 'Excellent LTV:CAC ratio' }
    ...
    case 'cacPaybackPeriod':
      if (value <= 12)
        return { score: 'excellent', message: 'Fast payback period' }
...
export default function SaaSMetricsDashboard() {
  ...
  const calculateMetrics = useCallback((currentMetrics: SaaSMetrics) => {
    ...
    const revenueChurnRate =
      (currentMetrics.churnedMrr / currentMetrics.mrr) * 100
    ...
    const churnedCustomers =
      currentMetrics.churnedMrr / currentMetrics.averageSellingPrice
    ...
    const ltv =
      currentMetrics.averageSellingPrice *
      avgCustomerLifetime *
      (currentMetrics.grossMargin / 100)
    ...
    const monthlyGrossRevenue =
      currentMetrics.averageSellingPrice * (currentMetrics.grossMargin / 100)
    const cacPaybackPeriod =
      monthlyGrossRevenue > 0 ? currentMetrics.cac / monthlyGrossRevenue : 0
    ...
    <div className="w-full max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          See how your key metrics interconnect. Change one number and watch the
          ripple effects.
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <Label htmlFor="averageSellingPrice">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateMetric('averageSellingPrice', e.target.value)
                }
                placeholder="100"
              />
            </div>
            ...
            {metrics.ltvCacRatio < 2 && (
              <div className="p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
                <p className="text-red-800 dark:text-red-300">
                  ⚠️ Your LTV:CAC ratio of {metrics.ltvCacRatio.toFixed(1)} is
                  below the healthy minimum of 2:1. Consider improving retention
            ...
            {metrics.cacPaybackPeriod > 18 && (
              <div className="p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <p className="text-yellow-800 dark:text-yellow-300">
                  ⏱️ CAC payback period of {metrics.cacPaybackPeriod.toFixed(1)}{' '}
                  months is longer than ideal. Focus on increasing ARPU or
            ...
            {metrics.churnRate > 5 && (
              <div className="p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
                <p className="text-orange-800 dark:text-orange-300">
                  📉 Monthly churn rate of {metrics.churnRate.toFixed(1)}% is
                  high. Improving retention by just 1% would increase your LTV
                  by {formatCurrency(metrics.ltv * 0.2)}.
                </p>
              </div>
            )}
            ...
            {metrics.mrrGrowthRate >= 15 && metrics.ltvCacRatio >= 3 && (
              <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg">
                <p className="text-green-800 dark:text-green-300">
                  🚀 Excellent metrics! You're in the top tier of SaaS
                  companies. Consider scaling your acquisition efforts.
                </p>
              </div>
            )}
          </div>
