...
export default function StartupRunwayCalculator() {
  ...
  const calculateMetrics = useCallback((currentMetrics: RunwayMetrics) => {
    ...
    newMetrics.currentRunway =
      currentMetrics.currentCash / currentMetrics.monthlyBurn
    ...
    const totalCashAfterFunding =
      newMetrics.cashAtFunding + currentMetrics.expectedFunding
    newMetrics.extendedRunway =
      newMetrics.runwayWithGrowth +
      totalCashAfterFunding / currentMetrics.monthlyBurn

    // Break even calculation
    let breakEvenMonths = 0
    let breakEvenRevenue = currentMetrics.monthlyRevenue
    while (
      breakEvenRevenue < currentMetrics.monthlyBurn &&
      breakEvenMonths < 60
    ) {
  ...
  useEffect(() => {
    ...
    metrics.expectedFunding,
    metrics.fundingTimeline,
    calculateMetrics,
  ])
  ...
  const calculateScenario = (scenario: Scenario) => {
    const adjustedGrowthRate =
      metrics.revenueGrowthRate * scenario.growthMultiplier
    ...
    <div className="w-full max-w-6xl mx-auto p-6 space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          Model your cash runway across different scenarios. Plan for the worst,
          hope for the best.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateMetric('revenueGrowthRate', e.target.value)
                }
                placeholder="15"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateMetric('expectedFunding', e.target.value)
                }
                placeholder="5000000"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateMetric('fundingTimeline', e.target.value)
                }
                placeholder="6"
              />
            </div>
            ...
            {metrics.runwayWithGrowth < 6 && (
              <div className="p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
                <p className="text-red-800 dark:text-red-300">
                  🚨 <strong>Critical:</strong> Only{' '}
                  {formatMonths(metrics.runwayWithGrowth)} runway remaining.
                  Immediate action required - cut costs, accelerate funding, or
                  pivot.
                </p>
              </div>
            )}
            ...
            {metrics.runwayWithGrowth >= 6 && metrics.runwayWithGrowth < 12 && (
              <div className="p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
                <p className="text-orange-800 dark:text-orange-300">
                  ...
                  {formatMonths(metrics.runwayWithGrowth)} runway. Start
                  fundraising now or implement cost reduction measures.
                </p>
              </div>
            )}
            ...
            {metrics.cashAtFunding < 0 && (
              <div className="p-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg">
                <p className="text-red-800 dark:text-red-300">
                  ...
                  Either accelerate funding by{' '}
                  {Math.abs(
                    Math.round(metrics.cashAtFunding / metrics.monthlyBurn)
                  )}{' '}
                  months or reduce burn rate.
                </p>
              </div>
            )}
            ...
            {metrics.breakEvenDate < metrics.runwayWithGrowth && (
              <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg">
                <p className="text-green-800 dark:text-green-300">
                  🎯 <strong>Great news:</strong> You'll reach break-even (
                  {formatMonths(metrics.breakEvenDate)}) before running out of
                  cash. Focus on maintaining growth trajectory.
                </p>
              </div>
            )}
            ...
            {metrics.revenueGrowthRate < 10 && (
              <div className="p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <p className="text-yellow-800 dark:text-yellow-300">
                  📈 Growth rate of {metrics.revenueGrowthRate}% is below
                  typical startup benchmarks. Consider focusing on growth
                  acceleration or extending runway through cost cuts.
                </p>
              </div>
            )}
            ...
            {metrics.runwayWithGrowth >= 18 && (
              <div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-blue-800 dark:text-blue-300">
                  ✅ <strong>Healthy runway:</strong>{' '}
                  {formatMonths(metrics.runwayWithGrowth)} gives you flexibility
                  to optimize for growth rather than survival. Consider
